...
void sglist_spills_to_heap_after_inline_capacity()
{
  std::array<rdma::mutable_buffer,
             rdma::detail::ibv_sglist_t::inline_sge_count + 1>
      buffers{};
  ...
  ASIO_CHECK(sglist.size() == buffers.size());
  ASIO_CHECK(sglist[0].addr ==
             reinterpret_cast<std::uintptr_t>(buffers[0].addr()));
  ASIO_CHECK(sglist[8].lkey == buffers[8].local_key());
}

void sglist_reuses_heap_capacity()
{
  rdma::detail::ibv_sglist_t sglist;
  sglist.resize(rdma::detail::ibv_sglist_t::inline_sge_count + 1);
  auto* heap = sglist.data();

  sglist.resize(rdma::detail::ibv_sglist_t::inline_sge_count);
  ASIO_CHECK(!sglist.uses_heap());
...
void builder_reports_all_empty_and_too_many_sge()
{
  ...
  rdma::detail::ibv_sglist_t empty_sglist;
  ...
  rdma::detail::ibv_sglist_t limited_sglist;
  auto limited =
