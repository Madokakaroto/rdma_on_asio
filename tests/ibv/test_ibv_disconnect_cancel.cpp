...
//
// Usage: test_ibv_disconnect_cancel [port].
...
bool phase_a(rdma::rdma_device_ptr const& device, std::string const& ip,
  ...
  asio::co_spawn(
      io,
      [&]() -> asio::awaitable<void> {
        auto [ecg, srv_conn, rqn] = co_await listener.async_get_connection(asio::mutable_buffer{}, nothrow);
        if (ecg) co_return;
...
bool phase_b(rdma::rdma_device_ptr const& device, std::string const& ip,
  ...
  asio::co_spawn(
      io,
      [&]() -> asio::awaitable<void> {
        auto [ecg, conn, rqn] = co_await listener.async_get_connection(asio::mutable_buffer{}, nothrow);
        if (ecg) co_return;
  ...
  asio::co_spawn(
      ...
      [&]() -> asio::awaitable<void> {
        auto ep = rdma_test::endpoint_for(ip, port);
        std::string req = "c";
        auto [ecc, rpn] = co_await cli.async_connect(qp_c, ep, asio::buffer(req), asio::mutable_buffer{}, nothrow);
        if (ecc) {
...
bool phase_c(rdma::rdma_device_ptr const& device, std::string const& ip,
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    c->open(rdma_test::port_space_for(ip));
    auto q = std::make_unique<rdma::rdma_queue_pair>(io);

    slot* sp = s.get();
    std::string req = "c";
    c->async_connect(*q, ep, asio::buffer(req), asio::mutable_buffer{},
                     [sp](asio::error_code ec, std::size_t) {
      sp->ec = ec;
...
int main(int argc, char* argv[]) {
  try {
    auto endpoint =
        rdma_test::query_local_endpoint_with_port_arg(argc, argv, 5012);
