// libibverbs echo test in POLL MODE. Same protocol as test_ibv_echo.cpp, but
// the data plane runs against a user-owned standalone ibv_completion_queue:
//
//   - The user creates an ibv_completion_queue and binds the queue pair to it
//     via the ibv_queue_pair(io, cq) constructor.
...
//   - The control plane (connect/accept/disconnect over rdma_cm) still runs on
//     the io_context; poll mode only changes how verbs-op completions arrive.
#include <array>
#include <atomic>
#include <iostream>
#include <span>
...
class cq_spinner {
public:
  explicit cq_spinner(rdma::ibv_completion_queue& cq)
      : cq_(cq), thread_([this] {
...
asio::awaitable<void> run_server(asio::io_context& io_ctx,
  ...
  rdma::ibv_queue_pair qp(io_ctx, cq);  // bind the QP to our poll-mode CQ
