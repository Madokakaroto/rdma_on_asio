...
//     io_context.
//   - Data plane runs against a user-owned ibv_completion_queue; the queue pair
//     is bound with ibv_queue_pair(cq) (no io_context). A dedicated thread spins
//     cq.poll(); data-plane ops use as_tuple(use_future) and their completions
//     fire inline on the poll thread (the QP uses system_executor in poll mode).
#include <array>
#include <atomic>
#include <future>
#include <iostream>
...
// after the connection is established (the QP exists), so QP creation never
// races with poll().
class cq_spinner {
public:
...
void run_server(asio::io_context& io_ctx, rdma::ibv_device_ptr const& device,
                uint16_t port) {
...
void run_client(asio::io_context& io_ctx, rdma::ibv_device_ptr const& device,
