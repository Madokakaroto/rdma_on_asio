...
template <mr_const_buffer_sequence BufferSequence, typename Handler,
          typename IoExecutor>
class rdma_send_op final : public rdma_two_sided_op<BufferSequence> {
public:
...
public:
  ASIO_DEFINE_HANDLER_PTR(rdma_send_op);
  rdma_send_op(asio::error_code const& ec,
               BufferSequence const& buffer_sequence, Handler& handler,
               IoExecutor const& io_ex)
      : base_type(&rdma_send_op::do_complete, ec, buffer_sequence)
...
private:
  static void do_complete(void* owner, rdma_verbs_op_base* base,
    ...
    rdma_send_op* o = static_cast<rdma_send_op*>(base);
    if (ec) {
