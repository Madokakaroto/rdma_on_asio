...
template <mr_mutable_buffer_sequence BufferSequence, typename Handler,
          typename IoExecutor>
class rdma_read_op final : public rdma_one_sided_op<BufferSequence> {
public:
...
public:
  ASIO_DEFINE_HANDLER_PTR(rdma_read_op);
  rdma_read_op(asio::error_code const& ec,
               BufferSequence const& buffer_sequence,
               rdma_remote_addr_t const& remote_addr, Handler& handler,
               IoExecutor const& io_ex)
      : base_type(&rdma_read_op::do_complete, ec, buffer_sequence, remote_addr)
...
private:
  ...
  static void do_complete(void* owner, rdma_verbs_op_base* base,
    ...
    rdma_read_op* o = static_cast<rdma_read_op*>(base);
