#ifndef RDMA_IBV_IMPL_IBV_OPS_CM_IPP
#define RDMA_IBV_IMPL_IBV_OPS_CM_IPP
...
bool set_nonblocking(int fd, asio::error_code& ec) {
...
native_event_channel_t* create_event_channel(asio::error_code& ec) {
...
int create_cm_id(native_event_channel_t* channel, native_cm_id_t** id,
...
int migrate_id(native_cm_id_t* cm_id, native_event_channel_t* channel,
               asio::error_code& ec) {
...
int bind_addr(native_cm_id_t* cm_id, sockaddr const* addr,
              asio::error_code& ec) {
  if (cm_id == nullptr) {
...
int listen(native_cm_id_t* cm_id, int backlog, asio::error_code& ec) {
  if (cm_id == nullptr) {
    ec = asio::error::bad_descriptor;
    return -1;
  }
...
int resolve_addr(native_cm_id_t* cm_id, sockaddr* src, sockaddr* dst,
...
int resolve_route(native_cm_id_t* cm_id, int timeout_ms,
                  asio::error_code& ec) {
  if (cm_id == nullptr) {
    ec = asio::error::bad_descriptor;
    return -1;
...
int connect(native_cm_id_t* cm_id, rdma_conn_param* conn_param,
            asio::error_code& ec) {
  if (cm_id == nullptr) {
    ec = asio::error::bad_descriptor;
    return -1;
...
int accept(native_cm_id_t* cm_id, rdma_conn_param* conn_param,
           asio::error_code& ec) {
  if (cm_id == nullptr) {
    ec = asio::error::bad_descriptor;
    return -1;
...
int disconnect(native_cm_id_t* cm_id, asio::error_code& ec) {
  if (cm_id == nullptr) {
    ec = asio::error::bad_descriptor;
    return -1;
  }
...
int get_cm_event(native_event_channel_t* channel,
...
void drain_cm_events(native_event_channel_t* channel,
                     native_cm_id_t* graceful_cm_id) {
  if (channel == nullptr) {
    return;
  }
  asio::error_code ec;
...
#endif  // RDMA_IBV_IMPL_IBV_OPS_CM_IPP
