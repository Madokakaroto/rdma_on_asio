#ifndef RDMA_IBV_IMPL_IBV_DEVICE_IMPL_IPP
#define RDMA_IBV_IMPL_IBV_DEVICE_IMPL_IPP
...
ibv_device_ptr create_device(native_context_t* context,
                             asio::error_code& ec) {
  if (!context) {
...
bool is_valid_device(ibv_device_ptr const& device) {
  return device && device->context_ != nullptr;
}

std::size_t sockaddr_size(sockaddr const* addr) noexcept {
  if (!addr) {
    return 0;
  }
...
std::optional<asio::ip::address> to_ip_address(sockaddr const* addr) {
...
bool is_candidate_local_address(ifaddrs const& interface) noexcept {
...
native_context_t* bindable_context_for(sockaddr const* addr) {
...
void attach_device_address(ibv_device_ptr const& device,
...
void attach_local_addresses(std::vector<ibv_device_ptr>& devices,
                            asio::error_code& ec) {
...
bool has_local_address(ibv_device_ptr const& device) noexcept {
...
std::vector<ibv_device_ptr> get_devices(asio::error_code& ec) {
...
std::vector<ibv_device_ptr> get_devices() {
  asio::error_code ec{};
...
#endif  // RDMA_IBV_IMPL_IBV_DEVICE_IMPL_IPP
