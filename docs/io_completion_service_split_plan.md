# Refactor Plan — split io_completion_service into device_service + io_completion_service

> Working/intermediate doc; delete after it lands. Not an interface change for users — this is an
...
1. **Device registration** — stores the user-supplied `device_ptr` + the derived
   `effective_config` (and the `is_initialized`/registered flag).
2. **Completion notification** — owns the shared CQ + comp_channel (epoll) / IOCP, registered on
...
- **`device_service`** — owns the registered `device_ptr` + `effective_config`; answers
  `is_registered()` / `get_device()` / `get_effective_config()`.
- **`io_completion_service`** — owns the shared CQ + comp_channel/IOCP notify + reactor poller +
...
void shutdown();                                       // reset
```
Pure storage — no reactor/scheduler. (`execution_context_service_base`.)
...
- **Drops** `device_`, `effective_config_`, `get_device()`, `get_effective_config()`, the
  ...
  `initialized_` flag** — "is set up?" is now answered solely by `device_service::is_registered()`.
...
roll back on CQ-init failure — rejected; the above is simpler and atomic.)

## Cached service references — no runtime `use_service` on hot paths

`asio::use_service<S>(ctx)` walks the service list comparing keys — non-trivial, and **must not
...
|---|---|---|---|
| `verbs_service` | `io_completion_service&` | `arm_notify()` — **per data-plane op** | `use_service` per op ❗ |
...
*before* and destroyed *after* the holder), and stays alive through the holder's `shutdown()` —
the same guarantee asio relies on for `reactor&`/`scheduler&`.

**Non-service caller — `queue_pair`** (not a service, so no ctor-init-list): the event-mode
...
> No new cycles: `device_service` depends on nothing; `io_completion_service` → reactor/scheduler;
> `verbs_service` → `io_completion_service`; `connector`/`listener` → `device_service`.

## Call-site migration

| current (`io_completion_service`) | → | new |
|---|---|---|
| `get_device()` | → | `device_service::get_device()` |
| `get_effective_config()` | → | `device_service::get_effective_config()` |
| `is_initialized()` used as the **"use_device called?" guard** | → | `device_service::is_registered()` (→ `ext_device_not_registered`) |
| `get_cq()` / `get_comp_channel()` | → | `io_completion_service` (unchanged) |
| `arm_notify()` | → | `io_completion_service` (unchanged) |
...
- **connector_service** (`ibv/nd_connector_service.hpp`): guard `is_registered()` +
  ...
  IND2Connector) → `device_service::get_device()`.
- **listener_service** (`ibv/nd_listener_service.hpp`): guard `is_registered()` from
  `device_service`. **nd also** uses `get_device()` (to create the listener) →
  `device_service::get_device()`.

`completion_queue` (poll) is untouched — it never used `io_completion_service`.

## File-by-file
- **new:** `include/ibv/detail/ibv_device_service.hpp`, `include/nd/detail/nd_device_service.hpp`.
- **edit:**
  - `ibv/nd_io_completion_service.hpp` — slim down (drop device/config/`is_initialized`;
    `initialize(device, cqe, ec)` self-guards on `cq_`).
  - `ibv/nd_use_device.hpp` — wire both services (derive config; `io.initialize` then
    `dev.register_device`).
  - `ibv/nd_queue_pair.hpp` — `bind` reads device/config from `device_service`, cq from
    `io_completion_service`; guard via `device_service::is_registered()`; **cache `verbs_service*`
    at `bind(io)`** for the per-op event path.
  - `ibv/nd_verbs_service.hpp` — hold `io_completion_service&` as a ctor-init-list member; use it
    in `arm_notify`/`finish_event` (no per-op `use_service`).
  - `ibv/nd_connector_service.hpp` + `ibv/nd_listener_service.hpp` — hold `device_service&` as a
...
2. Rewire `ibv_use_device`, `ibv_queue_pair::bind`, `ibv_connector_service`,
   `ibv_listener_service` — including the **cached-reference** conversions (verbs→io_completion,
   connector/listener→device_service, queue_pair caches verbs_service at bind). Build; run ibv +
   cross-platform echo (event + poll) over RoCE — no regression.
...
## Decisions
- **Q1 — [DECIDED]** keep the name `device_service` (per-io_context binding; distinct from the
  process-wide `device_manager` = discovery).
- **Q2 — [DECIDED: option C]** drop `io_completion_service::is_initialized()` + `initialized_`;
  `initialize()` self-guards on `cq_`; the single "use_device called?" predicate is
  `device_service::is_registered()`.
- **Q3 — [DECIDED]** `derive_effective_config` runs in `use_device` (atomic: init CQ first,
  register device only on success); `device_service` just stores the derived result.
- **Cross-cutting — [DECIDED]** service→service deps use a cached reference (member +
